 ...
 *
 */
public class DOTComplexDialog  implements DOTDefinitionDialog {
	 ...
	 */	
	public DOTComplexDialog( final DOTDefinitionDialogFrame dotDefinitionDialogFrame) {
		...
		if ( !(dotDefinitionDialogFrame.getDisplayObjectType() instanceof DOTComplex)) {
	...
	private void initStaticColorCenterPanel() {
		...
		if ( _dotDefinitionDialogFrame.isPropertyStatic( null, DOTProperty.FARBE)) {
			...
			_staticColorBox.getModel().setSelectedItem( DOTComplex.DEFAULT_COLOR_NAME);
		}
	...
	private void initANonStaticCenterPanel ( 
			...
			JPanel thePanel, 
			JTable theTable) {
		DOTComplex dotComplex = (DOTComplex) _dotDefinitionDialogFrame.getScratchDisplayObjectType();
		final DynamicDOTItemManager tableModel = (DynamicDOTItemManager) dotComplex.getTableModel( property);
	...
	public void saveDisplayObjectType() {
		DOTComplex newDOTComplex = (DOTComplex) _dotDefinitionDialogFrame.getScratchDisplayObjectType().getCopy( null);
		...
		newDOTComplex.setName( name);
		newDOTComplex.setInfo( _dotDefinitionDialogFrame.getInfoText());
		_dotDefinitionDialogFrame.getDotManager().saveDisplayObjectType(newDOTComplex);
		_dotDefinitionDialogFrame.setDisplayObjectType( newDOTComplex, true);
    }
	...
	private void addButtonListeners( final DOTProperty property,
		...
		ActionListener actionListenerShowConflictsButton = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				DOTComplex dotComplex = (DOTComplex) _dotDefinitionDialogFrame.getScratchDisplayObjectType();
				final Set<Integer> conflictingRows = dotComplex.getConflictingRows(property);
